...
using SquidEngine.Serialization;
...
namespace SquidEngine
{
    public static class SceneManager
    {
        ...
        private static SquidScene _activeScene;
        private static List<SquidScene> _scenes;
        private static List<SquidScene> _scenesToUnload;
        ...
        private static List<SquidFont> _embeddedFonts = new List<SquidFont>();
        ...
        public static List<SquidFont> EmbeddedFonts
        {
            get { return _embeddedFonts; }
            set { _embeddedFonts = value; }
        }

        public static List<SquidScene> Scenes
        {
            get { return _scenes; }
        }

        public static List<SquidScene> ScenesToUnload
        {
            get { return _scenesToUnload; }
            set { _scenesToUnload = value; }
        }

        public static SquidScene ActiveScene
        {
            ...
            set
            {
                ...
                SquidCore.activeSceneChanged = true;
            }
        }
        ...
        static SceneManager()
        {
            _scenes = new List<SquidScene>();
            _assemblies = new List<Assembly>();
            _globalDataHolder = new GlobalDataHolder();
            _scenesToUnload = new List<SquidScene>();
        ...
        private static bool HasComponents(Assembly asm)
        {
            foreach (Type t in asm.GetTypes())
            {
                ...
                foreach (var item in _attribs)
                {
                    ...
                    if (b.GetType().Equals(typeof(SquidComponentAttribute)))
                    {
        ...
        public static SquidScene LoadScene(string filename)
        {
        ...
        {
            InitializeGlobal(SquidEngine.Game.Instance.Services);
        }
        ...
        public static void LoadGlobalData(string rootPath)
        {
            String filename;
            if(rootPath == "")
                filename = Path.Combine(rootPath, "Content/SquidEngineAssets/global.squid");
            else
                filename = Path.Combine(rootPath, "../Content/SquidEngineAssets/global.squid");
        ...
        {
#if XBOX360
            _globalcontent = new ResourceContentManager(services, SquidResources.ResourceManager); //Add root directory ?
#else
			#if !XNATOUCH
            _globalcontent = new ResourceContentManager(services, SquidResources.ResourceManager); //Add root directory ?
#endif
            ...
            DrawingManager.EmbeddedSquidEffects[(int)EmbeddedSquidEffectType.Bloom]
                .Load(SceneManager.GlobalContent, bloomShaders);
            String[] gaussianShaders = { "GaussianBlurEffect" };
            DrawingManager.EmbeddedSquidEffects[(int)EmbeddedSquidEffectType.GaussianBlur]
            ...
            EmbeddedFonts.Add(new SquidFont("DefaultFont",
                _globalcontent.Load<SpriteFont>("Arial"), AssetScope.Embedded));

            EmbeddedFonts.Add(new SquidFont("DiagnosticFont",
        ...
        public static void InitializeGlobal(IServiceProvider services, bool loadMaterials)
        {
            ...
            if (loadMaterials == true)
            {
                ...
                foreach (SquidFont _mat in GlobalDataHolder.Fonts)
                {
                    _mat.Scope = AssetScope.Global;
                    if (_mat.Font != null)
                    {
				...
				#if !XNATOUCH
                foreach (SquidEffect _effect in GlobalDataHolder.Effects)
                {
        ...
        public static SquidFont GetEmbeddedFont(string name)
        {
            foreach (SquidFont _font in EmbeddedFonts)
            {
                if (_font.Name == name)
                {
                    return _font;
