...
namespace SquidEngine.Serialization
{
    ...
    public class SceneSerializer
    {
        ...
        {
            public const string ICESCENE = "SquidScene";
        ...
        private static XmlNode WriteAssets(XmlDocument doc, SceneBase scene)
        {
            ...
            foreach (SquidFont font in scene.Fonts)
            {
			...
			#if !XNATOUCH
            foreach (SquidEffect mat in scene.Effects)
            {
        ...
        private static void SerializeAsset(XmlDocument doc, XmlNode assetNode, SquidAsset asset)
        {
        ...
        {
            SerializeAsset(doc, materialNode, (SquidAsset)material);
        ...
        private static void WriteSceneComponent(XmlNode componentNode, SquidSceneComponent comp, XmlDocument doc)
        {
            foreach (var _property in comp.GetType().GetProperties())
            {
                if (_property.GetCustomAttributes(typeof(SquidEngine.Attributes.SquidComponentPropertyAttribute), true).Length > 0)
                {
        ...
        private static void WriteSceneComponent(XmlNode sceneComponent, SquidComponent comp, XmlDocument doc)
        {
            foreach (var _property in comp.GetType().GetProperties())
            {
                if (_property.GetCustomAttributes(typeof(SquidEngine.Attributes.SquidComponentPropertyAttribute), true).Length > 0)
        ...
        private static string GetAssetSource(SquidAsset asset)
        {
        ...
        private static void WriteTileGrid(XmlNode itemNode, XmlDocument doc, TileGrid tileGrid)
        {
            ...
            XmlNode _passable = _gridSettings.AppendChild(doc.CreateElement("Passable"));
            string passableText = "";
            ...
            passableText = passableText.TrimEnd(',');
            _passable.InnerText = passableText;
        }
        ...
        private static XmlNode WriteProperty(string name, object item, XmlDocument doc, object defaultValue)
        {
            ...
            if (_prop.CanRead && _prop.CanWrite)
            {
                ...
                else if (_prop.PropertyType == typeof(SquidEngine.Drawing.SquidColor))
                {
                    _node.InnerText = ParseColor((SquidEngine.Drawing.SquidColor)_prop.GetValue(item, null));
                }
                else if (_prop.PropertyType == typeof(Rectangle?))
                {
        ...
        public static void DeSerializeScene(String filename, SceneBase scene)
        {
            ...
            if (scene is GlobalDataHolder)
            {
                ...
                XmlNode _resNode = doc.SelectSingleNode("SquidScene/NativeResolution");
                if (_resNode != null)
                {
        ...
        public static SquidScene DeSerializeScene(String filename, String rootPath)
        {
            SquidScene scene = new SquidScene();
        ...
        {
            XmlNode _componentsNode = doc.SelectSingleNode("SquidScene/Components");
            if (_componentsNode == null)
                return;

            List<SquidSceneComponent> _components = new List<SquidSceneComponent>();
            foreach (XmlNode _compNode in _componentsNode.ChildNodes)
            {
                ...
                SquidSceneComponent _comp = CreateSceneComponentInstance(_compNode.Name);
                if (_comp != null)
                {
                    ...
                    _comp.SetOwner(scene as SquidScene);
        ...
        private static void LoadTemplateItems(XmlDocument doc, SceneBase scene)
        {
            try
            {
                XmlNode _el = doc.SelectSingleNode("SquidScene/Templates");
        ...
        private static void LoadSceneItems(XmlDocument doc, SceneBase scene)
        {
            try
            {
                XmlNode _el = doc.SelectSingleNode("SquidScene/SceneItems");
        ...
        private static void LoadSceneCollection(SceneBase scene, XmlNode _el, List<SceneItem> sceneitems)
        {
            try
            {
                ...
                foreach (XmlNode _node in _el.ChildNodes)
                {
                    ...
                    _item = LoadSceneItem(_node, scene as SquidScene);
        ...
        private static SceneItem LoadSceneItem(XmlNode node, SquidScene scene)
        {
            SceneItem _item = null;
        ...
        private static SceneItem LoadPostProcessAnimation(XmlNode _node, SceneBase scene)
        {
            ...
            {
                foreach (var item in DrawingManager.EmbeddedSquidEffects)
                {
        ...
        private static void LoadTileGridSettings(XmlNode gridsettings, TileGrid tilegrid)
        {
            ...
            XmlNode _passable = gridsettings.SelectSingleNode("Passable");
            if (_passable != null)
            {               
                string [] s = _passable.InnerText.Split(',');
                int count = 0;
                ...
                for (int y = 0; y < tilegrid.TileRows; ++y)
                {
                    for (int x = 0; x < tilegrid.TileCols; ++x)
                    {
                        ...
                        if (s[count] == "1")
                            ...
                            Passable = false;
                        tilegrid.TileLayers[0].Tiles[x][y].Passable = Passable;
                        ++count;
                    }
                }
            }
        ...
        static SquidScene _storedScene;
        private static SceneItem LoadCompositeEntity(XmlNode node, SceneBase scene)
        {
            ...
            _storedScene = scene as SquidScene;
        ...
        private static void LoadEmitter(XmlNode emitterNode, ParticleEffect particle, SceneBase scene)
        {
            try
            {
                ...
                particle.Emitter = new SquidEngine.SceneItems.ParticlesClasses.Emitter();
        ...
        private static void SetProperty(string p, object targetObject, XmlNode node, object defaultValue)
        {
            ...
            if (_newNode == null)
            {
                ...
                {
                    _prop.SetValue(targetObject, new SquidEngine.Drawing.SquidColor(), null);
                }
                return;
            }
            ...
            {
                SquidEngine.Drawing.SquidColor newColor = (SquidEngine.Drawing.SquidColor)ParseToSquidColor(_newNode.InnerText);
                _prop.SetValue(targetObject, newColor, null);
            }
            else if (_prop.PropertyType == typeof(bool))
            {
        ...
        private static SquidFont LoadFontFromNode(XmlNode _node, SceneBase scene)
        {
                ...
                if (_prop != null)
                {
                    ...
                    if (_prop.PropertyType.IsGenericType)
                    {
                        ...
                        if (_prop.Name == "Components")
                        {
                            ...
                            List<SquidComponent> _components = new List<SquidComponent>();
                            foreach (XmlNode _compNode in _node.ChildNodes)
                            {
                                TraceLogger.TraceInfo("Load Component");
                                TraceLogger.TraceInfo(_compNode.Name);
                                Components.SquidComponent _comp = CreateComponentInstance(_compNode.Name);
                                if (_comp == null)
        ...
        private static SquidEngine.Components.SquidComponent CreateComponentInstance(string p)
        {
            return (SquidComponent)ComponentTypeContainer.CreateNewInstance(p);
        }
		#if !XNATOUCH
        private static SquidEffect CreateEffectInstance(string p)
        {
            return (SquidEffect)ComponentTypeContainer.CreateNewInstance(p);
        }
#endif

        private static SquidSceneComponent CreateSceneComponentInstance(string p)
        {
            SquidSceneComponent comp1 = (SquidSceneComponent)ComponentTypeContainer.CreateNewInstance(p);
            return comp1;
        }

        private static object ParseToSquidColor(string color)
        {
            string[] vals = color.Split(',');
#if(WINDOWS)
            SquidEngine.Drawing.SquidColor col = new SquidEngine.Drawing.SquidColor(
        ...
        private static string ParseColor(SquidEngine.Drawing.SquidColor color)
        {
            string s = string.Format("{0},{1},{2},{3}", color.R, color.G, color.B, color.A);
            return s;
        }
        ...
        private static void LoadAssets(XmlDocument doc, SceneBase scene)
        {
            ...
            try
            {
                XmlNode _el = doc.SelectSingleNode("SquidScene/Assets");
                int _count = _el.ChildNodes.Count;
                int _current = 0;
                foreach (XmlNode _node in _el.ChildNodes)
                {
                    ...
                    if (_node.Name.ToUpper() == "MATERIAL")
                    {
                        ...
                        if (_node.Attributes.GetNamedItem("areas_definition") != null)
                        {
                            ...
                            if (SceneSerializer._rootPath == "")
                                definitionPath = newMat.AreasDefinitionFilename.Replace("../", "");
                            else
                                definitionPath = Path.Combine(SceneSerializer._rootPath, newMat.AreasDefinitionFilename);
                    ...
                    {
                        asset = new SquidFont();
                        scene.Fonts.Add(asset as SquidFont);
                    ...
                    else if (_node.Name.ToUpper() == "EFFECT")
                    {
                        ...
                        scene.Effects.Add(asset as SquidEffect);
                    ...
                    // if not using squid editor, remove the ../
                    if (SceneSerializer._rootPath == "")
