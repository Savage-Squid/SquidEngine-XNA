...
{
    public class BloomEffect : SquidEffect
    {
        #region Fields
        ...
        public override void Draw(SpriteBatch spriteBatch)
        {
            ...
            //return;
            // Pass 1: draw the scene into rendertarget A, using a
            ...
            // Pass 2: draw from rendertarget A into rendertarget B,
            // using a shader to apply a horizontal gaussian blur filter.
            SetBlurEffectParameters(1.0f / (float)DrawingManager.ViewPortSize.X, 0);
            DrawingManager.RenderTargetManager.SwitchTo(RenderTargetInstance.B);
            GraphicsDevice.Clear(Color.Black);
            ...
            DrawFullScreenQuad(spriteBatch, DrawingManager.RenderTargetManager.GetTexture(RenderTargetInstance.A),
                Effects[1]);            
            // Pass 3: draw from rendertarget B back into rendertarget A,
            // using a shader to apply a vertical gaussian blur filter.
            SetBlurEffectParameters(0, 1.0f / (float)DrawingManager.ViewPortSize.Y);
            DrawingManager.RenderTargetManager.SwitchTo(RenderTargetInstance.A);
            GraphicsDevice.Clear(Color.Black);
            //DrawingManager.RenderTargetManager.ResolveToTexture(RenderTargetInstance.B);
            DrawFullScreenQuad(spriteBatch, DrawingManager.RenderTargetManager.GetTexture(RenderTargetInstance.B), 
                Effects[1]);
