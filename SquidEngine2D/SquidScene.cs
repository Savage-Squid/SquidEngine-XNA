...
{
    public class SquidScene : SceneBase
    {
        ...
        /// <remarks>An SquidScene should always have at least 1 camera.</remarks>
        /// </summary>
        ...
        /// <summary>
        /// Instiantiates an instance of SquidScene
        /// </summary>
        public SquidScene()
        {
        ...
        public T CreateCopy<T>(string name) where T : SceneItem, new()
        {
            ...
            foreach (SceneItem _obj in TemplateItems)
            {
                if (_obj.Name == name && _obj.IsTemplate)
                {
                    ...
                    foreach (SquidComponent _comp in _obj.Components)
                    {
                        _newObject.AddComponent((SquidComponent)_comp.GetCopy());
                    }                    
        ...
        /// </summary>
        /// <typeparam name="T">The generic type of SquidSceneComponent to get</typeparam>
        /// <returns></returns>
        public T GetComponent<T>() where T : SquidSceneComponent
        {
            T local = default(T);
            foreach (SquidSceneComponent component in SceneComponents)
            {
