...
namespace SquidEngine
{
    public class ComponentTypeContainer
    {
            ...
            foreach (Type t in types)
            {
                ...
                foreach (var item in _attribs)
                {
                    ...
                    if (b.GetType().Equals(typeof(SquidComponentAttribute)) 
                        || b.GetType().Equals(typeof(SquidEffectAttribute)))
                    {
                        _types.Add(t, _id);
                    }
                }
        ...
        internal static object DeepCopySquidSceneComponent(Type type, SquidSceneComponent cloneFrom)
        {
            //Create a new instance of the component
            SquidSceneComponent comp = (SquidSceneComponent)type.Assembly.CreateInstance(type.FullName);
            //CopyPropertiesTo(comp, cloneFrom);
            cloneFrom.CopyValuesTo(comp);
            return comp;
        }
