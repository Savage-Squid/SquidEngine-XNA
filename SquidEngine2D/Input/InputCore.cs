...
namespace SquidEngine.Input
{
    ...
    public static class InputCore
    {
        ...
        internal static void Update(float elapsed)
        {
            ...
            SquidProfiler.StartProfiling(SquidProfilerNames.ICE_CORE_INPUT_UPDATE);
            ...
            SquidProfiler.StopProfiling(SquidProfilerNames.ICE_CORE_INPUT_UPDATE);
        }
        ...
        public static bool IsNewKeyDown(Keys key)
        {
            for(int i=0; i<4; ++i)
            {
                if(IsNewKeyDown((PlayerIndex)i, key))
                    return true;
            }
            return false;
        }

        public static bool IsNewKeyDown(PlayerIndex playerIndex, Keys key)
        {
            return _lastKBState[(int)playerIndex].IsKeyUp(key) &&
                _currentKBState[(int)playerIndex].IsKeyDown(key);
        }
