...
{
    public enum SquidEditorPreferencesEditorStartupAction
    {
        DoNothing,
    ...
    public class SquidEditorPreferences
    {
        #region Static Methods

        public static string GetPath()
        {
            return Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData)+ "\\SquidEngine\\settings.xml";            
        }

        internal static SquidEditorPreferences Load()
        {
            try
            {
                XmlSerializer _ser = new XmlSerializer(typeof(SquidEditorPreferences));
                if (!File.Exists(GetPath()))
                    return new SquidEditorPreferences();
                Stream _stream = File.Open(GetPath(), FileMode.Open);
                SquidEditorPreferences pref = (SquidEditorPreferences)_ser.Deserialize(_stream);
                _stream.Close();
                return pref;
            }
            catch (Exception err)
        ...
        public SquidEditorPreferencesEditorStartupAction EditorStartupAction
        {
            get;
            set;
        }
        ...
        public int SelectedGrid
        {
            ...
            set
            {
                _selectedGrid = SquidMath.Clamp(value, 1, 3);
            }
        }
        ...
        public SquidEditorPreferences()
        {
            this.EditorStartupAction = SquidEditorPreferencesEditorStartupAction.OpenLastEditedProject;
