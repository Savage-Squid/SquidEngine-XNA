...
namespace SquidEditor.Editors.AnimatedSprites
{
    public partial class AnimatedSpriteEditor : SceneItemEditor
    {
        ...
        private void AnimatedSpriteEditor_Load(object sender, EventArgs e)
        {
            ...
            pictureBoxTint.BackColor = SquidEditorForm.GetGDIColor(this.AnimatedSprite.Tint);
        ...
        private void OpenTintSelectionDialog()
        {
            ...
            colorDialog.Color = SquidEditorForm.GetGDIColor(this.AnimatedSprite.Tint);
        ...
        private void buttonSelectTexture_Click(object sender, EventArgs e)
        {
                ...
                && this.AnimatedSprite.Material != materialSelectorDialog.SelectedMaterial)
            {
                ...
                {
                    SquidEditorForm.ShowErrorMessage("The selected Material \"" + 
        ...
        private void listViewAnimations_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            toolStripButtonAddAllFrames.Enabled = enabledState;
            toolStripButtonChangeAllDurations.Enabled = enabledState;
        ...
        private void toolStripButtonAddAllFrames_Click(object sender, EventArgs e)
        {
            if ( this.SelectedAnimationInfo != null)
            {
                // Clear the current key frames
                this.SelectedAnimationInfo.AnimationFrames.Clear();
                LoadFrameList();

                // Add all of the key frames
                String[] areas = GetAreasList();
                foreach (String area in areas)
                {
                    this.SelectedAnimationInfo.AnimationFrames.Add(new AnimationFrame(10, area));
                }
                LoadFrameList();
            }
        }

        private void toolStripButtonChangeAllDurations_Click(object sender, EventArgs e)
        {
            if (this.SelectedAnimationInfo != null)
            {
                if (this.SelectedAnimationInfo.AnimationFrames.Count > 1)
                {
                    for (int i = 1; i < this.SelectedAnimationInfo.AnimationFrames.Count; ++i)
                    {
                        SelectedAnimationInfo.AnimationFrames[i] = new AnimationFrame(this.SelectedAnimationInfo.AnimationFrames[0].Duration, 
                            this.SelectedAnimationInfo.AnimationFrames[i].Area);
                    }
                    LoadFrameList();
                }
            }
