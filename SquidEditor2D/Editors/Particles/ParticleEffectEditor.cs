...
namespace SquidEditor.Editors.Particles
{
    public partial class ParticleEffectEditor : SceneItemEditor
    {
        ...
        private void OpenLoadBackgroundDialog()
        {
            ...
            {           
                ...
                newTexture = Texture2D.FromStream(particleEffectControl.GraphicsDevice, fs);
                fs.Close();
        ...
        private void toolStripButtonDelPartType_Click(object sender, EventArgs e)
        {
            ...
            if (index != -1)
            {
                if (SquidEditorForm.ShowWarningQuestion("Do you really want to delete this particle type?") 
                    == true)
                {
            ...
            if (shapeSelection.ShowDialog() == DialogResult.OK)
            {
                ...
                if (shape.Type == EmitterShapeType.TextureMask)
                {
                    try
                    {
                        ...
                        System.IO.FileStream fs = new System.IO.FileStream(shapeSelection.TexturePath, System.IO.FileMode.Open);
                        Texture2D maskTexture = Texture2D.FromStream(DrawingManager.GraphicsDevice, fs);
                        fs.Close();
