...
namespace SquidEditor.Editors.TileGrids
{
    ...
    public partial class TileGridEditorControl : GraphicsDeviceControl
    {
        ...
        private Texture2D _impassableTexture;
        ...
        {                      
            ...
            fs.Close();
            fs = new System.IO.FileStream(Application.StartupPath + "\\Resources\\check_green-small.png", System.IO.FileMode.Open);
            _passableTexture = Texture2D.FromStream(GraphicsDevice, fs);
            fs.Close();
            fs = new System.IO.FileStream(Application.StartupPath + "\\Resources\\x_red-small.png", System.IO.FileMode.Open);
            _impassableTexture = Texture2D.FromStream(GraphicsDevice, fs);
            fs.Close();
        ...
        protected void ApplyPassageToggle(int x, int y)
        {
            int layer = 0;
            bool Passable = TileGrid.TileLayers[0].Tiles[x][y].Passable;
            TileGrid.TileLayers[layer].Tiles[x][y].Passable = !Passable;
        }
        ...
        {
            tileCoord.X = SquidMath.Clamp(tileCoord.X, 0, TileGrid.TileCols - 1);
            tileCoord.Y = SquidMath.Clamp(tileCoord.Y, 0, TileGrid.TileRows - 1);
            return tileCoord;
        }
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left)
            {
                ...
                if (PaintMode == TileGridPaintMode.Brush)
                {
                    ...
                    {
                        ApplyBrushPaint(TileGridEditor.Instance.SelectedBrushTiles);
                    }
                }
                else if (PaintMode == TileGridPaintMode.Eraser)
                {
                    if (_brushStartPoint.X > -1 && _brushStartPoint.Y > -1)
                    {
                        List<int> EraserBrush = new List<int>();
                        EraserBrush.Add(-1);
                        ApplyBrushPaint(EraserBrush);
                        EraserBrush.Clear();
                        EraserBrush = null;
                    }
                }
        ...
        protected override void OnMouseClick(MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left)
            {
                ...
                else if (PaintMode == TileGridPaintMode.Bucket)
                {
                    ...
                    if (_brushStartPoint.X > -1 && _brushStartPoint.Y > -1)
                    {
                        ApplyFillBucketPaint(_brushStartPoint.X, _brushStartPoint.Y, TileGridEditor.Instance.SelectedBrushTiles[0]);
                    }
                }
                else if (PaintMode == TileGridPaintMode.Passage)
                {
                    ...
                    if (_brushStartPoint.X > -1 && _brushStartPoint.Y > -1)
                    {
                        ApplyPassageToggle(_brushStartPoint.X, _brushStartPoint.Y);
                    }
                }
                else if (PaintMode == TileGridPaintMode.Edit)
                {
        ...
        private void StoreCopyOfSelectedTiles()
        {
            ...
            {                
                ...
                {
                    topLeft.X = SquidMath.Min(topLeft.X, _selectedTiles[i].X);
                    topLeft.Y = SquidMath.Min(topLeft.Y, _selectedTiles[i].Y);
                }
        ...
        protected void DrawBrushTileHighlight(Color color)
        {
            ...
            {                         
                ...
                clampedSize.X = SquidMath.Clamp(_brushStartPoint.X + clampedSize.X,
                    0, TileGrid.TileCols) - _brushStartPoint.X;
                clampedSize.Y = SquidMath.Clamp(_brushStartPoint.Y + clampedSize.Y,
